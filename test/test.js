...
contract("dBank", ([deployer, user]) => {
  ...
  describe("testing token contract...", () => {
    describe("success", () => {
      it("should have a token name", async () => {
        expect(await token.name()).to.be.eq("My Currency");
      });

      it("should have a token symbol", async () => {
        expect(await token.symbol()).to.be.eq("MC");
      });

      it("should have a token initial total supply of 0", async () => {
